...
class OpenRouterTest(unittest.TestCase):
    ...
    def test_explains_parameter_routing_error_requires_logprobs(self):
        ...
        with patch(
            "urllib.request.urlopen", side_effect=unavailable_parameters
        ) as urlopen:
            with self.assertRaisesRegex(
                RuntimeError,
                "no provider.*requires logprobs and top_logprobs.*logprob-capable",
            ):
                query_openrouter(
                    "vendor/model", "Question", 5, 100, "test-key"
                )
